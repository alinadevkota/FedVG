...
class KvasirCapsule(BaseDataset):
    def __init__(
        self,
        root,
        args=None,
        ...
        # We'll use PIL images, not preload into memory
        dataset = ImageFolderWithPaths(root=image_dir)

        self.classes = dataset.classes
        self.class_to_idx = dataset.class_to_idx
        self.samples = dataset.samples
        self.imgs = dataset.imgs
        self.targets = torch.tensor([label for _, label in dataset.samples])

        # We pass paths and labels instead of preloading tensors
        super().__init__(
            data=[path for path, _ in dataset.samples],
    ...
    def __getitem__(self, index):
        ...
        if self.train and self.train_data_transform:
            image = self.train_data_transform(image)
        else:
            image = self.test_data_transform(image)
        target = self.targets[index]
        return image, target  # ✅ remove path


class DermaMnist(BaseDataset):
    def __init__(
        self,
        root,
        args=None,
        ...
        train_part = DermaMNIST(root=root, split="train", download=True, size=224)
        test_part = DermaMNIST(root=root, split="test", download=True, size=224)
        print(train_part.imgs.shape)
        train_data = torch.Tensor(train_part.imgs).permute([0, -1, 1, 2]).float()
        test_data = torch.Tensor(test_part.imgs).permute([0, -1, 1, 2]).float()
        train_targets = torch.Tensor(train_part.labels).long().squeeze()
        test_targets = torch.Tensor(test_part.labels).long().squeeze()
