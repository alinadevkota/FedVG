...
class FedGradDynClient(FedAvgClient):
    ...
    def fit(self):
        ...
        for _ in range(self.local_epoch):
            for x, y in self.trainloader:
                ...
                loss = loss_ce + self.args.fedgraddyn.alpha * loss_correct
                self.optimizer.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(
                    self.model.parameters(), max_norm=self.args.fedgraddyn.max_grad_norm
                )
                self.optimizer.step()
