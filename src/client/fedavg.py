...
class FedAvgClient:
    def __init__(
        ...
        # Please don't bother with the [0], which is only for avoiding raising runtime error by setting Subset(indices=[]) with `DataLoader(shuffle=True)` # noqa: E501
        ...
        for split, color, flag, subset in [
            ...
            if len(subset) > 0 and flag:
                eval_msg.append(
                    ...
                    f"[blue]accuracy: {eval_results['before'][split].accuracy:.2f}% -> {eval_results['after'][split].accuracy:.2f}%[/blue]"  # noqa: E501
                )
    ...
    def package(self):
        ...
        Returns:
            A dict: {
                ...
                `model_params_diff`: The parameter difference between the client trained and the global. `diff = global - trained`. # noqa: E501
