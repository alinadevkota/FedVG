...
class FedGradElasticClient(FedAvgClient):
    ...
    def load_data_indices(self):
        ...
        sampled_size = int(
            len(train_data_indices) * self.args.fedgradelastic.sample_ratio
        )
    ...
    def train(self, server_package: dict[str, Any]):
        ...
        for x, y in self.sampled_trainloader:
            ...
            for i in range(len(grad_norms)):
                if grad_norms[i]:
                    sensitivity[i] = (
                        self.args.fedgradelastic.mu * sensitivity[i]
                        + (1 - self.args.fedgradelastic.mu) * grad_norms[i].abs()
                    )
                else:
                    sensitivity[i] = 1.0
