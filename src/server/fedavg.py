...
def get_dataset_name(name):
    if name.startswith("cifar10"):
        return "cifar10"
    elif name.startswith("tiny_imagenet"):
        return "tiny_imagenet"
    elif name.startswith("kvasir_capsule"):
        return "kvasir_capsule"
    elif name.startswith("covid19"):
        return "covid19"
    elif name.startswith("medmnistA"):
        return "medmnistA"
    elif name.startswith("dermamnist"):
        return "dermamnist"
...
class FedAvgServer:
    def __init__(
        ...
        Args:
            ...
            `return_diff`: `True` indicates that clients return `diff = W_global - W_local` as parameter update; `False` for `W_local` only. # noqa: E501
        """
        self.args = args
        ...
        self.val_noise_rate = self.args.common.val_noise_rate
        self.val_subset_ratio = self.args.common.val_subset_ratio
        self.val_imbalance_ratio = self.args.common.val_imbalance_ratio

        self.realistic_split = self.args.common.realistic_split
        ...
        self.dataset_name = get_dataset_name(self.args.dataset.name)
        ...
        if self.args.common.monitor == "visdom":
            ...
            self.viz = Visdom(
                server=args.common.visdom_server, port=str(args.common.visdom_port)
            )
        ...
        (
            ...
            self.val_noise_rate,
            self.val_subset_ratio,
            self.val_imbalance_ratio,
            self.realistic_split,
            self.dataset_name,
        )
    ...
    def get_dataset(self) -> BaseDataset:
        ...
        """
        dataset: BaseDataset = DATASETS[get_dataset_name(self.args.dataset.name)](
    ...
    def get_dataset_transforms(self):
        ...
        #     [
        #         # commenmted out for debug jun19# transforms.Resize((DATA_SHAPE[self.dataset_name][-2], DATA_SHAPE[self.dataset_name][-1])), # noqa: E501
        #         transforms.Normalize(
        #             DATA_MEAN[self.dataset_name], DATA_STD[self.dataset_name]
        #         )
        #     ]
        #     if self.dataset_name in DATA_MEAN and self.dataset_name in DATA_STD
        #     else []
        # )
        # test_data_transform = []
        ...
        # following lines were necessary because the results are better without normalization
        train_target_transform = transforms.Compose([])
        train_data_transform = transforms.Compose([])
        test_target_transform = transforms.Compose([])
        test_data_transform = transforms.Compose([])

        if self.dataset_name == "kvasir_capsule":
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("resize reached")
            train_data_transform = transforms.Compose(
                [
                    transforms.Resize(
                        (
                            DATA_SHAPE[self.dataset_name][-2],
                            DATA_SHAPE[self.dataset_name][-1],
                        )
                    )
                ]
            )
        ...
        elif self.args.model.name.startswith("vit"):
            train_data_transform = transforms.Compose(
                [
                    transforms.Resize(
                        (
        ...
        for split, client_side_test_flag, server_side_test_flag in [
            ...
            for stage in ["before", "after"]:
                if client_side_test_flag:
                    ...
                    if self.args.common.monitor == "visdom":
                        ...
                        self.wandb_run.log(
                            {
                                f"Accuracy-{self.monitor_window_name_suffix}/{split}set-{stage}LocalTraining": aggregated.accuracy,  # noqa: E501
            ...
            if (
                ...
                if self.args.common.monitor == "visdom":
                    ...
                    self.wandb_run.log(
                        {
                            f"Accuracy-{self.monitor_window_name_suffix}/{split}set-CentralizedEvaluation": self.test_results[  # noqa: E501
                                self.current_epoch + 1
                            ][
                                "centralized"
                            ][
    ...
    def run_experiment(self):
        ...
        if self.args.common.monitor == "tensorboard":
            ...
            self.wandb_run.log(
                {
                    f"ExperimentalArguments-{self.monitor_window_name_suffix}": json.dumps(
                        OmegaConf.to_object(self.args), indent=4
                    )
                }
            )
            ...
            epoch: {
                group: {
                    split: {
                        ...
                        "macro_f1": f"[blue]{metrics['before'][split].macro_f1:.2f}% -> "
                        f"{metrics['after'][split].macro_f1:.2f}%[/blue]",
                        "micro_f1": f"[blue]{metrics['before'][split].micro_f1:.2f}% -> "
                        f"{metrics['after'][split].micro_f1:.2f}%[/blue]",
                        "mcc": f"[blue]{metrics['before'][split].mcc:.2f}% -> "
                        f"{metrics['after'][split].mcc:.2f}%[/blue]",
                    }
                    for split, flag in [
                        (
                            "train",
        ...
        if self.args.common.monitor == "tensorboard":
            ...
            for epoch, results in all_test_results.items():
                self.wandb_run.log(
                    {
                        f"Results-{self.monitor_window_name_suffix}": f"<pre>{results}</pre>",
                        "epoch": epoch,
                    }
                )
