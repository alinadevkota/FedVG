...
class ViT(DecoupledModel):
    archs = {
        "b_16": (models.vit_b_16, models.ViT_B_16_Weights.DEFAULT),
        "b_32": (models.vit_b_32, models.ViT_B_32_Weights.DEFAULT),
        "l_16": (models.vit_l_16, models.ViT_L_16_Weights.DEFAULT),
        "l_32": (models.vit_l_32, models.ViT_L_32_Weights.DEFAULT),
    }
    timm_archs = {"s_16": "vit_small_patch16_224"}
    swin_archs = {
    ...
    def __init__(self, version, dataset, pretrained=True):
        super().__init__()

        if version in self.archs:
            vit_model_fn, vit_weights = self.archs[version]
            weights = vit_weights if pretrained else None
            vit: models.VisionTransformer = vit_model_fn(weights=weights)

            # vit_model_fn = self.archs[version]
            # vit: models.VisionTransformer = vit_model_fn(weights=None)

            self.base = vit
            self.classifier = nn.Linear(
                self.base.heads.head.in_features, NUM_CLASSES[dataset]
            )
            self.base.heads.head = nn.Identity()
        elif version in self.swin_archs:
            model_fn, weights_enum = self.swin_archs[version]
            weights = weights_enum if pretrained else None
            model = model_fn(weights=weights)
            self.base = model
            in_features = model.head.in_features
            self.base.head = nn.Identity()
            self.classifier = nn.Linear(in_features, NUM_CLASSES[dataset])
        elif version in self.timm_archs:
            model_name = self.timm_archs[version]
            vit = timm.create_model(model_name, pretrained=pretrained)
            self.base = vit
            self.classifier = nn.Linear(
                self.base.head.in_features, NUM_CLASSES[dataset]
            )
            self.base.head = nn.Identity()
...
MODELS = {
    ...
    "vit_b_16": partial(ViT, version="b_16"),
    "vit_b_32": partial(ViT, version="b_32"),
    "vit_l_16": partial(ViT, version="l_16"),
    "vit_l_32": partial(ViT, version="l_32"),
    "vit_s_16": partial(ViT, version="s_16"),
    "vit_swin_s": partial(ViT, version="swin_s"),
    "vit_swin_t": partial(ViT, version="swin_t"),
}
