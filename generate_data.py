...
def main(args):
    dataset_root = (
        CURRENT_DIR / "data" / f"{args.dataset}_nc{args.client_num}_alpha{args.alpha}"
    )
    ...
    if args.dataset == "femnist":
        ...
        if len(target_indices) > 0:
            if args.alpha > 0:  # Dirichlet(alpha)
                ...
                print("randomly assigning classes")
